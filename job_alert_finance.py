#!/usr/bin/env python3
# job_alert_finance.py - sister's version of job_alert.py.
# Targets entry-level Finance / Business / Product / Data-analyst roles
# for a BS in Finance and Technology Management (NYU, Class of May 2027).
# Same sources and pipeline as job_alert.py; only the matching rules,
# email recipient, and state filenames differ, so both scripts can run
# side by side in this repo without touching each other's memory.
...
EMAIL_RECIPIENTS = ["x221gaye@gmail.com"]
...
# Tech companies still matter here: they all hire financial analysts,
# BizOps, and product people. Fintech/finance boards added on top.
# A wrong slug just 404s once and gets muted for a week, so guesses
# are cheap - add any company whose careers page uses these platforms.
GREENHOUSE_COMPANIES = ["datadog", "mongodb", "squarespace",
                        "justworks", "betterment", "peloton",
                        "duolingo", "stripe", "affirm", "chime",
                        "sofi", "robinhood", "gusto", "point72",
                        "twosigma"]
LEVER_COMPANIES = ["spotify", "plaid", "palantir"]
ASHBY_COMPANIES = ["ramp", "openai", "notion", "runwayml", "mercury"]
...
# borderline posting and answers: "Would a Bachelor's in Finance and
# Technology Management (Class of 2027) reasonably qualify?" Costs
# pennies/day with Haiku. Leave blank for pure keyword scoring (free).
ANTHROPIC_API_KEY = ""
LLM_MODEL = "claude-haiku-4-5-20251001"
MAX_LLM_JOBS = 48            # cost/safety cap per run

# --- Smart filter tuning --------------------------------------------
# The Simplify new-grad feed is tech-heavy (Software, AI/ML/Data,
# Hardware, Quant, Product). For a finance/business profile only the
# Product listings are relevant as jobs, but the WHOLE feed is still
# used to auto-discover hundreds of company boards - those same
# companies post the finance/BizOps roles we actually want.
SIMPLIFY_CATEGORIES = ["Product", "Product Management"]
...
SEEN_FILE = BASE_DIR / "seen_jobs_finance.json"
DEAD_FILE = BASE_DIR / "dead_boards_finance.json"
DEAD_BOARD_RETRY_DAYS = 7
LOG_FILE = BASE_DIR / "log_finance.txt"
...
# Same machinery as job_alert.py, but every list is rebuilt around a
# BS in Finance and Technology Management. Function/variable names are
# kept identical so the rest of the pipeline is untouched: here
# "tech_score" measures finance/business fit, not CS fit.

TECH_QUALIFIERS = (
    "finance", "financial", "investment", "investments", "banking",
    "credit", "risk", "treasury", "equity", "equities", "markets",
    "capital markets", "trading", "wealth", "asset", "assets",
    "portfolio", "fund", "funds", "valuation", "underwriting",
    "pricing", "revenue", "billing", "insurance", "actuarial",
    "business", "commercial", "corporate", "strategy", "strategic",
    "operations", "product", "project", "program", "data",
    "analytics", "business intelligence", "bi", "compliance",
    "regulatory", "fintech", "payments", "economic", "economics",
    "research", "client", "implementation", "technology", "digital",
    "transformation", "consulting", "advisory", "audit", "accounting",
    "tax",
)
ROLE_NOUNS = ("analyst", "associate", "consultant", "specialist",
              "coordinator", "administrator", "accountant", "auditor",
              "planner", "researcher")
STANDALONE_TECH_TITLES = (
    # finance-core
    "financial analyst", "finance analyst", "investment banking analyst",
    "investment analyst", "credit analyst", "risk analyst",
    "treasury analyst", "equity research", "research analyst",
    "portfolio analyst", "fp&a", "financial planning",
    "corporate finance", "banking analyst", "capital markets analyst",
    "sales and trading analyst", "asset management analyst",
    "wealth management analyst", "private equity analyst",
    "venture capital analyst", "fund analyst", "fund administrat",
    "valuation analyst", "m&a analyst", "compliance analyst",
    "kyc analyst", "aml analyst", "middle office", "fund accountant",
    # business / tech-management side of her degree
    "business analyst", "product analyst", "data analyst",
    "operations analyst", "strategy analyst", "pricing analyst",
    "revenue analyst", "billing analyst", "consulting analyst",
    "associate consultant", "technology analyst",
    "business technology", "solutions analyst",
    "implementation analyst", "product operations",
    "business operations", "associate product manager",
    "product management associate", "rotational program",
    "rotation program", "analyst program",
    "leadership development program", "graduate analyst",
    "project coordinator", "program coordinator", "project analyst",
)
TITLE_EXCLUDES = (
    "intern", "internship", "co-op", "coop", "senior", "sr", "staff",
    "principal", "lead", "director", "manager", "vp",
    "vice president", "chief", "executive", "head of",
    # in banking, "Associate" above analyst level is MBA-track:
    "investment banking associate", "private equity associate",
    "m&a associate",
    "recruiter", "recruiting", "talent acquisition",
    "human resources", "hr", "attorney", "legal", "paralegal",
    "counsel", "nurse", "nursing", "physician", "dental", "clinical",
    "therapist", "pharmacist", "marketing", "sales",
    "account executive", "business development", "customer service",
    "cashier", "retail", "barista", "server", "waiter", "warehouse",
    "forklift", "driver", "delivery", "construction", "carpenter",
    "electrician", "plumber", "hvac", "mechanic", "janitor",
    "custodian", "housekeeping", "security guard", "teacher", "tutor",
    "phd",
    # engineering roles belong to the CS script, not this one:
    "engineer", "engineering", "developer", "programmer", "scientist",
    "architect", "devops", "sre", "designer",
)
STACK_WORDS = (
    "excel", "financial modeling", "financial model", "valuation",
    "dcf", "lbo", "comps", "pitch", "bloomberg", "factset",
    "capital iq", "morningstar", "gaap", "financial statements",
    "balance sheet", "income statement", "cash flow", "forecasting",
    "budgeting", "variance analysis", "reconciliation",
    "journal entries", "month-end close", "cfa", "sie", "finra",
    "series 7", "series 63", "series 79", "kyc", "aml",
    "due diligence", "equities", "fixed income", "derivatives",
    "credit analysis", "underwriting", "risk management",
    "capital markets", "asset management", "portfolio management",
    "mergers", "investor relations", "financial analysis",
    "profitability", "revenue recognition", "sox",
    "internal controls", "treasury", "liquidity", "market research",
    "kpi", "dashboards", "powerpoint", "sql", "tableau", "power bi",
    "python", "erp", "sap", "netsuite", "quickbooks", "jira",
    "agile", "scrum", "stakeholder", "business requirements",
    "process improvement", "project management",
)
STACK_SUBSTRINGS = ("m&a", "fp&a")

CS_FIELD_RE = re.compile(
    r"\bfinance\b|accounting|economics|business administration|"
    r"business analytics|information systems|"
    r"management information|\bbusiness\b.{0,20}degree|"
    r"related (business )?field|stem degree", re.I)
...
LEVEL_ONE_RE = re.compile(
    ...
    r"accountant|auditor)\s+(i|1)\b", re.I)
BACHELOR_RE = re.compile(
    r"bachelor|\bb\.?b\.?a\.?\b|"
    r"\bb\.?s\.?\b.{0,30}(degree|finance|business|accounting|econom)|"
...
# Jobs whose description demands a Master's/PhD/MBA with no
# Bachelor's path are out of reach for a BS-only Class-of-2027 grad.
GRAD_DEGREE_RE = re.compile(
    r"\bph\.?\s?d\b|doctorate|doctoral|\bmaster'?s?\b|\bm\.s\.|"
    r"\bmsc\b|\bmba\b|graduate degree", re.I)
BACHELOR_OK_RE = re.compile(
    r"bachelor|\bb\.?s\.?\b|\bb\.?a\.?\b|\bb\.?b\.?a\.?\b|"
    r"undergraduate", re.I)

# Signals that a posting is really an engineering / clinical / other
# off-degree role; these pull the fit score DOWN.
NONTECH_WORDS = (
    "software engineer", "software development", "programming",
    "coding", "kubernetes", "docker", "terraform",
    "distributed systems", "machine learning engineer",
    "deep learning", "pytorch", "tensorflow", "microservices",
    "backend", "frontend", "full stack", "compiler", "verilog",
    "fpga", "circuit design", "solidworks", "autocad", "clinical",
    "patient care", "registered nurse", "juris doctor",
    "cpa required", "cpa license required", "mba required",
    "cold calling", "sales quota", "commission-based",
)
...
HARD_EXCLUDES = ("intern", "internship", "co-op", "coop", "senior",
                 "sr", "staff", "principal", "lead", "director", "vp",
                 "vice president", "chief", "head of")


def title_excluded(title):
    t = title.lower()
    # Whitelisted phrases (e.g. "associate product manager") beat the
    # generic blacklist, but never override seniority/intern markers.
    if any(s in t for s in STANDALONE_TECH_TITLES):
        return any(_word(t, h) for h in HARD_EXCLUDES)
...
def local_verdict(job):
    ...
    if not job["needs_marker"]:
        # Simplify jobs are pre-vetted new-grad. Only the Product
        # category comes through (see SIMPLIFY_CATEGORIES), so skip
        # the generic blacklist ("manager" would wrongly kill a
        # new-grad Product Manager) and check seniority + a loose
        # business/product title only.
        t = title.lower()
        if any(_word(t, h) for h in HARD_EXCLUDES):
            return "no"
        loose = (any(s in t for s in STANDALONE_TECH_TITLES)
...
LLM_QUESTION = (
    "You screen job postings for a student earning a Bachelor of "
...
def src_remotive(seen, cutoff):
    ...
    if not INCLUDE_REMOTE:
        return []
    jobs = []
    for cat in ("finance-legal", "product", "data"):
...
def src_usajobs(seen, cutoff):
    ...
    if not USAJOBS_API_KEY or not USAJOBS_EMAIL:
        return []
    params = urlencode({
        # 0501 financial admin / 0560 budget / 1101 general business /
        # 1102 contracting / 1160 financial analysis / 0343 mgmt analysis
        "JobCategoryCode": "0501;0560;1101;1102;1160;0343",
...
def src_adzuna():
    ...
    for where in searches:
        params = {"app_id": ADZUNA_APP_ID, "app_key": ADZUNA_APP_KEY,
                  ...
                  "sort_by": "date", "category": "accounting-finance-jobs",
                  "what_or": ("financial analyst finance investment "
                              "banking credit risk treasury business "
                              "graduate junior entry")}
        if where:
            params["where"] = where
        else:
...
def bucket(title):
    t = title.lower()
    if any(k in t for k in ("financ", "invest", "banking", "credit",
                            "risk", "treasury", "equity", "capital",
                            "wealth", "asset", "fund", "portfolio",
                            "trading", "audit", "account", "tax",
                            "underwrit", "insurance", "compliance")):
        return "software"          # bucket key kept; heading says Finance
    if any(k in t for k in ("data", "analytics", "business intelligence",
                            "reporting")):
        return "data"
    return "other"                 # product, ops, strategy, consulting
...
def build_email(jobs, extra_count=0):
    ...
    html_body = ("<html><body style='font-family:Arial,sans-serif'>"
                 f"<h2>{len(jobs)} entry-level finance &amp; business "
                 "posting(s)</h2>"
                 "<p style='color:#555'>Roles a Finance &amp; Technology "
                 "Management grad (Class of 2027) can apply for, "
                 "newest first.</p>"
                 + html_section("&#128176; Finance &amp; Banking",
                                groups["software"])
                 + html_section("&#128200; Business, Product &amp; Ops",
                                groups["other"])
                 + html_section("&#128202; Data &amp; Analytics",
                                groups["data"])
                 + more +
                 "<p style='color:#999;font-size:12px'>Sent automatically by "
                 "job_alert_finance.py.</p></body></html>")

    lines = [f"{len(jobs)} entry-level finance & business posting(s):", ""]
    for j in jobs:
        loc = ", ".join(j["locations"]) or "Location not listed"
    ...
    subject = (f"{len(jobs)} entry-level finance & business posting(s) - "
               f"{datetime.now():%b %d, %I:%M %p}")
    if not jobs:
        subject = (f"Finance job alerts ran - nothing new "
                   f"({datetime.now():%b %d})")
